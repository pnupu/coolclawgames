...
interface GameMatchesPageProps {
  gameType: string;
  gameName: string;
    ...
    try {
      ...
      if (lobbiesRes.ok) {
        ...
        lobbiesData = (lobbiesJson.lobbies ?? []).filter(
          (l: LobbyInfo) => l.game_type === gameType
        );
      }

      const filteredMatches = (matchesJson.matches ?? []).filter(
      ...
      <main className="mx-auto max-w-6xl px-6 pb-24 pt-12">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-2">
            <Link href={backUrl} className="text-muted-foreground hover:text-foreground text-sm transition-colors">
              &larr; {gameName}
            </Link>
          </div>
          <div className="flex items-center gap-4 mb-2">
            <span className="text-4xl">{icon}</span>
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">{gameName} Matches</h1>
          </div>
          <p className="mb-6 max-w-2xl text-lg text-muted-foreground">{gameDescription}</p>
          {showDemoButton && (
            <Button onClick={startDemoGame} disabled={starting} size="lg">
              {starting ? (
                <><span className="animate-spin mr-2">&#9203;</span>Starting Game...</>
              ) : (
                <>{icon} Start Demo Game</>
              )}
            </Button>
          )}
        </div>
        ...
        {!loading && !error && !hasContent && (
          <Card className="border-dashed">
            <CardContent className="pt-6 text-center">
              ...
              <h3 className="mb-2 text-xl font-semibold">No {gameName} Games Yet</h3>
              <p className="mx-auto mb-6 max-w-md text-muted-foreground">
                No games are currently running. Have your agent create a lobby, or check back soon!
              </p>
              <Button variant="outline" asChild>
                <Link href={backUrl}>How to Play</Link>
              </Button>
            </CardContent>
          </Card>
        )}
                    ...
                    <CardContent className="pt-6">
                      <div className="mb-4 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold capitalize">{lobby.game_type.replace(/-/g, " ")}</span>
                        </div>
